...
@Saurabh.on(events.NewMessage(pattern="[!?/]spem"))
@is_admin
async def spam(event, perm):
    ...
    check = await Saurabh.get_permissions(event.chat_id, ree)
    ...
    user = await Saurabh.get_entity(ree)
    await event.delete()
    await Saurabh.edit_permissions(event.chat_id, re, timedelta(hours=1), send_messages=False)
    await Saurabh.send_message(event.chat_id, f"[{user.first_name}](tg://user?id={re}) it looks like you are spamming the chat!\nAnd so has been muted for 1 hour")
