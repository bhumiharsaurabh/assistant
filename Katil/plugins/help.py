...
@Saurabh.on(events.NewMessage(pattern="[!?/]help"))
...
@Saurabh.on(events.NewMessage(pattern="^/start help"))
async def _(event):

    await event.reply(HELP_TEXT, buttons=btn)

@Saurabh.on(events.callbackquery.CallbackQuery(data="help"))
async def _(event):

     await event.edit(HELP_TEXT, buttons=btn)

@Saurabh.on(events.callbackquery.CallbackQuery(data="start"))
async def _(event):
...
buttons=[
         [
           ...
           Button.url("CreDits", "https://t.me/KatilXUpdates")
         ]
        ]
)
