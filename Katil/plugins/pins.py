...
@Saurabh.on(events.NewMessage(pattern="^[?!/]pinned"))
async def get_pinned(event):
    chat_id = (str(event.chat_id)).replace("-100", "")

    Ok = await Saurabh.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
@Saurabh.on(events.NewMessage(pattern="^[!?/]pin ?(.*)"))
@is_admin
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await Saurabh.pin_message(event.chat_id, msg, notify=True)
       return
    await Saurabh.pin_message(event.chat_id, msg)   

@Saurabh.on(events.NewMessage(pattern="^[!?/]unpin ?(.*)"))
@is_admin
...
async def permapin(event, perm):
    ...
    if not msg:
       ...
       return
    hn = await Saurabh.send_message(event.chat_id, msg.message)
    await Saurabh.pin_message(event.chat_id, hn, notify=True)


@Saurabh.on(events.NewMessage(pattern="^[!?/]unpinall"))
...
"""
    ...
    await Saurabh.send_message(event.chat_id, UNPINALL, buttons=[
    [Button.inline("Confirm", data="unpin")], 
    [Button.inline("Cancel", data="cancel")]])

@Saurabh.on(events.callbackquery.CallbackQuery(data="unpin"))
async def confirm(event):
    check = await event.client.get_permissions(event.chat_id, event.sender_id)
    if check.is_creator:
        await Saurabh.unpin_message(event.chat_id)
        await event.edit("Unpinned All Msgs!")
        return 

    await event.answer("Group Creator Required!")

@Saurabh.on(events.callbackquery.CallbackQuery(data="cancel"))
...
@Saurabh.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
